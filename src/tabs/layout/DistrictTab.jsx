...
const DIVIDER = { height:"1px",
  background:"rgba(255,255,255,0.1)", margin:"20px 0" };
const VAR_LBL = {
  ...
  return (
    <svg width={S} height={S} viewBox={`0 0 ${S} ${S}`}
      ...
      <rect x="1" y="1" width={S-2} height={S-2} fill="none"
        ...
        strokeWidth="1" rx="1"/>
      {dir==="left"  &&<line x1="1.5"   y1="1.5"   x2="1.5"   y2={S-1.5}
        stroke={C} strokeWidth={T} strokeLinecap="round"/>}
      {dir==="top"   &&<line x1="1.5"   y1="1.5"   x2={S-1.5} y2="1.5"
        stroke={C} strokeWidth={T} strokeLinecap="round"/>}
      {dir==="right" &&<line x1={S-1.5} y1="1.5"   x2={S-1.5} y2={S-1.5}
        stroke={C} strokeWidth={T} strokeLinecap="round"/>}
      {dir==="bottom"&&<line x1="1.5"   y1={S-1.5} x2={S-1.5} y2={S-1.5}
        stroke={C} strokeWidth={T} strokeLinecap="round"/>}
    </svg>
  );
}
...
function RulerIcon({ active }) {
  ...
  return (
    <svg width="15" height="15" viewBox="0 0 20 20" fill="none">
      ...
      <line x1="4"  y1="6" x2="4"  y2="10" stroke={c}
        strokeWidth="1.2"/>
      <line x1="8"  y1="6" x2="8"  y2="9"  stroke={c}
        strokeWidth="1.2"/>
      <line x1="12" y1="6" x2="12" y2="10" stroke={c}
        strokeWidth="1.2"/>
      <line x1="16" y1="6" x2="16" y2="9"  stroke={c}
        strokeWidth="1.2"/>
    </svg>
  );
}
...
function AreaInput({ label, colKey, rowKey, cfg, onChange }) {
  ...
  const inp = f => ({
    background:"transparent", border:"none",
    borderBottom:`1px solid ${f ? EMERALD :
      "rgba(255,255,255,0.22)"}`,
  ...
  return (
    <div style={{display:"flex", alignItems:"flex-end", gap:"16px"}}>
      <span style={{fontSize:"12px", fontWeight:600,
        ...
        flexShrink:0, minWidth:"110px", paddingBottom:"5px"}}>{label}
      </span>
      <div style={{flex:1, display:"flex", flexDirection:"column",
        gap:"3px"}}>
        <span style={VAR_LBL}>Column</span>
        <input type="number" value={cfg[colKey]||""} placeholder="0"
          onChange={e => onChange(colKey,
            e.target.value==="?"?parseInt(e.target.value,10)||0)}
          onFocus={()=>setFcol(true)} onBlur={()=>setFcol(false)}
          style={inp(fcol)}/>
      </div>
      <div style={{flex:1, display:"flex", flexDirection:"column",
        gap:"3px"}}>
        ...
        <input type="number" value={cfg[rowKey]||""} placeholder="0"
          onChange={e => onChange(rowKey,
            e.target.value==="?"?parseInt(e.target.value,10)||0)}
          onFocus={()=>setFrow(true)} onBlur={()=>setFrow(false)}
          style={inp(frow)}/>
      </div>
    </div>
  );
...
function HierarchyRow({ name, nameColor, value, onChange,
  onRulerToggle, onDelete, isCenter=false }) {
  return (
    <div style={{display:"grid", gridTemplateColumns:COLS, gap:"10px",
      alignItems:"flex-end"}}>
      {/* 1. 이름 */}
      <div style={{display:"flex", alignItems:"center", gap:"5px",
        ...
        {!isCenter && (
          <div style={{width:"8px", height:"8px", borderRadius:"2px",
            flexShrink:0,
            background:nameColor?.fill, border:`1px solid ${
              nameColor?.stroke}`}}/>
        )}
      ...
      <div style={{display:"flex", justifyContent:"center",
        ...
        {!isCenter && (
          <button onClick={onRulerToggle}
            ...
            style={{
              background: value.ruler ? "rgba(56,189,248,0.18)" :
                "rgba(255,255,255,0.05)",
              border:`1px solid ${value.ruler ? "rgba(56,189,248,0.5)" :
      ...
      <div style={{display:"flex", justifyContent:"center",
        ...
        {!isCenter && (
          <button onClick={onDelete}
            ...
            onMouseLeave={e =>
              e.currentTarget.style.color="rgba(255,80,80,0.45)"}>×</button>
        )}
      </div>
    </div>
  );
...
function buildAreas(col, row, margins, K) {
  ...
  for (let d=0; d<margins.length; d++) {
    ...
    if(dT>0)       p("T", vL,     0,    vR, vT, cw-dL-dR, dT);
  ...
  areas.push({name:"CTR", ox, oy, w:cw, h:ch, rW:cw, rH:ch,
    c:CENTER_C});
  return areas;

  function p(name,x,y,w,h,rW,rH,c:cc) {
...
function DistrictPreview({ totalCol, totalRow, margins, scale,
  previewWidth }) {
  const col = parseInt(totalCol)||0, row = parseInt(totalRow)||0;
  if (!col||!row) return (
    <div style={{display:"flex", alignItems:"center",
      justifyContent:"center",
      height:"100px", color:"rgba(255,255,255,0.2)", fontSize:"11px",
      letterSpacing:"1px"}}>
      ...
      {areas.map((a,i) => {
        ...
        return (
          <g key={i}>
            ...
            {pw>20&&ph>14&&(
              <>
                <text x={px+pw/2} y={py+ph/2-fs*0.55}
                  textAnchor="middle"
                  fill={a.c.text} fontSize={fs} fontWeight="700"
                  fontFamily="monospace">
                  {a.name}
                </text>
                <text x={px+pw/2} y={py+ph/2+fs*1.0}
                  textAnchor="middle"
...
export default function DistrictTab({ cfg, onChange }) {
  const [inputMargin, setInputMargin] =
    useState({left:"",top:"",right:"",bottom:""});
  ...
  const deleteMargin = i => onChange("districts", margins.filter((_,j) =>
    j!==i));

  const legendItems = [
    ...margins.map((_,i) => ({label:`D${i+1}`,
    ...
    <div style={{display:"flex", gap:"36px", alignItems:"flex-start"}}>
      ...
      <div style={{flex:"0 0 420px", display:"flex", flexDirection:"column"}}>
        ...
        <div style={{display:"flex", flexDirection:"column", gap:"16px"}}>
          <AreaInput label="Total Area [ea]"  colKey="total_col"
            rowKey="total_row"   cfg={cfg} onChange={onChange}/>
          <AreaInput label="Actv. Dum. [ea]"  colKey="actv_dum_col"
            rowKey="actv_dum_row" cfg={cfg} onChange={onChange}/>
        </div>

        <div style={DIVIDER}/>

        <p style={SEC_LBL}>Hierarchy</p>
        <AddRow value={inputMargin} onChange={setInputMargin}
          onAdd={addMargin}/>
        ...
        <div style={{display:"flex", flexDirection:"column", gap:"14px"}}>
          ...
          <HierarchyRow name="CTR" isCenter value={{}}
            onChange={null}/>
        </div>
      </div>
...
rgba(255,255,255,0.1)",
            ...
            <span style={{fontSize:"9px", color:"rgba(255,255,255,0.35)",
              ...
              flexShrink:0}}>Display Scale</span>
            <input type="number" min="1" value={scale}
              onChange={e => setScale(Math.max(1,
          ...
          <div style={{display:"flex", gap:"10px", flexWrap:"wrap",
            ...
            {legendItems.map((item, i) => (
              <div key={i} style={{display:"flex", alignItems:"center",
                ...
                <span style={{fontSize:"10px", color:item.c.text,
